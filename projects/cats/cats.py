...
def about(topic):
    ...
    "*** YOUR CODE HERE ***"
    def exist(paragraph):
        a = split(remove_punctuation(lower(paragraph)))
        for x in topic:
            if x in a:
                return True
        return False
...
def accuracy(typed, reference):
    ...
    if total == 0:
        return 0.0
    right = 0
    for i in range(min(total, len(reference_words))):
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in valid_words:
        return user_word
    mindiff_word = valid_words[0]
    mindiff = diff_function(user_word, mindiff_word, limit)
    for word in valid_words:
        currdiff = diff_function(user_word, word, limit)
        if currdiff < mindiff:
            mindiff_word, mindiff = word, currdiff
    if mindiff <= limit:
        return mindiff_word
    return user_word
    # END PROBLEM 5
...
def shifty_shifts(start, goal, limit):
    ...
    """
    # BEGIN PROBLEM 6
    def tracker(start, goal, limit, sofar):
        if sofar > limit:
            return sofar
        if len(start) == 1 or len(goal) == 1:
            if start[0] == goal [0]:
                return sofar + abs(len(start) - len(goal))
            else:
                return sofar + abs(len(start) - len(goal)) + 1
        else:
            if start[0] == goal [0]:
                return tracker(start[1:], goal[1:], limit, sofar)
            else:
                return tracker(start[1:], goal[1:], limit, sofar + 1)
    return tracker(start, goal, limit, 0)
    # END PROBLEM 6
...
def fastest_words(game):
    ...
    for i_words in words:
        i_fastest_player = 0
        for i_players in players:
            if time(game, i_players, i_words) < time(game, i_fastest_player, i_words):
                i_fastest_player = i_players
        lst[i_fastest_player] += [word_at(game, i_words)]
    return lst
    # END PROBLEM 10
