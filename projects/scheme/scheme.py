...
def eval_all(expressions, env):
    ...
    """
    # BEGIN PROBLEM 7
    # expressions_cp, buffer = expressions, None
    # if expressions_cp == nil:
    #     return None
    # while expressions_cp:
    #     buffer = scheme_eval(expressions_cp.first, env)
    #     expressions_cp = expressions_cp.rest
    # return buffer
    # END PROBLEM 7
    expressions_cp = expressions
    if expressions_cp == nil:
        return None
    while expressions_cp.rest != nil:
        scheme_eval(expressions_cp.first, env)
        expressions_cp = expressions_cp.rest
    return scheme_eval(expressions_cp.first, env, True)
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('Incorrect number of arguments  to funtion call')
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        fms = self.formals
        vals = args
        child = self.env.make_child_frame(fms, vals)
        while fms:
            child.define(fms.first, vals.first)
            fms, vals = fms.rest, vals.rest
        return child
        # END PROBLEM 11
...
def do_and_form(expressions, env):
    ...
    #     if is_false_primitive(curr_value):
    #         return False
    #     else:
    ...
    if expressions == nil:
        return True
    while expressions:
        curr_value = scheme_eval(expressions.first, env, expressions.rest == nil)
        if is_false_primitive(curr_value) or expressions.rest == nil:
            return curr_value
        else:
            expressions = expressions.rest
    return curr_value
...
def do_or_form(expressions, env):
    ...
    # while expressions:
    #     curr_value = scheme_eval(expressions.first, env)
    #     if is_true_primitive(curr_value):
    #         return curr_value
    #     else:
    #         expressions = expressions.rest
    # return False
    ...
    while expressions:
        curr_value = scheme_eval(expressions.first, env, expressions.rest == nil)
        if is_true_primitive(curr_value) or expressions.rest == nil:
            return curr_value
        else:
            expressions = expressions.rest
    return False
...
def make_let_frame(bindings, env):
    ...
    "*** YOUR CODE HERE ***"
    while bindings:
        binding_curr = bindings.first
        validate_form(binding_curr, 2, 2)
        names, values = Pair(binding_curr.first, names), Pair(scheme_eval(binding_curr.rest.first, env), values)
...
def do_define_macro(expressions, env):
    ...
    name_formals = expressions.first
    if isinstance(name_formals, Pair):
        name, formals, body = name_formals.first, name_formals.rest, expressions.rest
        if scheme_symbolp(name):
            validate_formals(formals)
            env.define(name, MacroProcedure(formals, body, env))
            return name
    raise SchemeError('cannot locate name and formals')
    # END Problem 20
