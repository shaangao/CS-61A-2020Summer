...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 9
            occupying = place.ant
            if occupying.can_contain(self):
                occupying.contain_ant(self)
                place.ant = occupying
            elif self.can_contain(occupying):
                self.contain_ant(occupying)
                place.ant = self
            else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        curr_place, i = self.place, 0
        bee_curr_place = rANTdom_else_none(curr_place.bees)
        while (i < self.min_range or not bee_curr_place) and i < self.max_range and curr_place.entrance != beehive:
            curr_place, i = curr_place.entrance, i + 1
            bee_curr_place = rANTdom_else_none(curr_place.bees)
            if bee_curr_place and self.min_range <= i <= self.max_range:
                return bee_curr_place 
        return bee_curr_place
        # END Problem 3 and 4
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        self.queen_count = QueenAnt.queen_count + 1
    ...
    def action(self, gamestate):
        ...
        if self.queen_count != 1:
            ...
            while effect_place:
                if effect_place.ant:
                    if effect_place.ant.damage_doubled == False:
                        effect_place.ant.damage *= 2
                        effect_place.ant.damage_doubled = True
                    if isinstance(effect_place.ant, ContainerAnt) and effect_place.ant.contained_ant:
                        if effect_place.ant.contained_ant.damage_doubled == False:
                            effect_place.ant.contained_ant.damage *= 2
                            effect_place.ant.contained_ant.damage_doubled = True
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if target and target.scared_before == False:
