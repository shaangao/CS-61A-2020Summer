...
def interleaved_sum(n, odd_term, even_term):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(curr, tracker, sofar):
        if curr > n:
            return sofar
        if curr <= n and tracker == 1:
            return helper(curr + 1, tracker * (-1), sofar + odd_term(curr))
        elif curr <= n and tracker == -1:
            return helper(curr + 1, tracker * (-1), sofar + even_term(curr))
    return helper(1, 1, 0)
