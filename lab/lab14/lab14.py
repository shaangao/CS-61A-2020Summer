...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            ...
            nmatch = skel_char + match
            possibilities.append((nmatch, cost))
        # Insert
        match, cost = helper_align(skeleton_idx, code_idx + 1)
        nmatch = '+[' + code_char + ']' + match
        possibilities.append((nmatch, cost + 1))
        # Delete
    ...
    def count(s, diff):
        if not s:
            if abs(diff) <= d:
                ...
                return 0
        return count(s[1:], diff + s[0]) + count(s[1:], diff - s[0])
    return count(s, 0) // 2
