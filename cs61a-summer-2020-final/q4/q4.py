...
class GroceryList:
    ...
    def buy_item(self, identifier):
        ...
        """
        i = 0
        while i in range(len(self.items)):
            item = self.items[i]
    ...
    def buy_cheapest(self):
        ...
        [tomato, chicken, oreos]
        """
        cheapest = min(self.items, key = Item.price)
    ...
    def generate_list(self, pesos_available, types):
        ...
        spent = 0
        i = 0
        while i in range(len(self.items)) and spent < pesos_available:
            item = self.items[i]
            if item.type in types and item.price <= pesos_available:
                spent += item.price
                yield item
            i += 1
